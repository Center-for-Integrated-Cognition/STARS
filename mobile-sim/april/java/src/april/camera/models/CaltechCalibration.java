...
public class CaltechCalibration implements Calibration, ParameterizableCalibration
{
    ...
    public String getCalibrationString()
    {
        ...
        s = String.format("%s            // focal length fx, fy\n", s);
        s = String.format("%s            fc = [%11.6f,%11.6f ];\n", s, fc[0], fc[1]);
        s = String.format("%s            // focal center cx, cy\n", s);
        s = String.format("%s            cc = [%11.6f,%11.6f ];\n", s, cc[0], cc[1]);

        s = String.format("%s            // radial distortion parameters\n", s);
        s = String.format("%s            // equivalent to kc(1), kc(2), kc(5)... in Caltech toolbox\n", s);
        s = String.format("%s            // equivalent to k1, k2, k3... in OpenCV\n", s);
        s = String.format("%s            kc = [",s);
        for (int i = 0; i < LENGTH_KC; i++)
            s = String.format("%s%11.6f%s", s, kc[i], (i+1 < LENGTH_KC) ? "," : " ];\n");

        s = String.format("%s            // tangential distortion parameters\n", s);
        s = String.format("%s            // equivalent to kc(3) and kc(4) in Caltech model\n", s);
        s = String.format("%s            // equivalent to p1, p2 in OpenCV model\n", s);
