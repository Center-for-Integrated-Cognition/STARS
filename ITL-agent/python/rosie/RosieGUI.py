...
class RosieGUI(Frame):
    class MessagesList(Text):
        """ Displays a list of messages that are the interaction history """
        def __init__(self, parent):
            Text.__init__(self, parent, font=("Times", "16"), takefocus=0)
            self.parent = parent
    ...
    class ScriptList(Listbox):
        ...
        def on_script_double_click(self, event):
            ...
            self.parent.fill_message_to_rosie(message)
    ...
    class ChatEntry(Entry):
        ...
        def __init__(self, parent):
            Entry.__init__(self, parent, font=("Times", "18"))
            self.parent = parent
    ...
    def setup_rosie_client(self):
        ...
        self.rosie_client.add_event_handler(AgentMessageSent, lambda e: self.messages_list.add("Rosie: " + e.message))
        self.rosie_client.add_event_handler(InstructorMessageSent, lambda e: self.messages_list.add("Instructor: " + e.message))
    ...
    def create_widgets(self, show_scripts):
        ...
        self.submit_button = Button(self, text="Send", font=("Times", "20"))
    ...
    def save_dialog(self):
        text = self.messages_list.get("1.0",END)
        f = open("dialog.txt", "w")
        f.write(text)
        f.close()
    ...
    def fill_message_to_rosie(self, message):
        if len(message) > 0:
            self.chat_entry.clear()
            self.chat_entry.fill_message(message)
