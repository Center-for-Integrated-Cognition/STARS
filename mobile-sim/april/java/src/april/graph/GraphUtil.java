...
public class GraphUtil
{
    ...
    public static class SolveState
    {
        ...
        public CholeskyDecomposition cd;
    ...
    public static Matrix getMarginalCovarianceRanged(Graph g, int nodeid)
    {
        ...
        int nodeDim = ess.g.nodes.get(nodeid).getDOF();
        int stateIdx = ess.g.getStateIndex(nodeid);

        int n = ess.g.getStateLength();
        Matrix I_part = new Matrix(n, nodeDim, Matrix.SPARSE);
        for (int i = 0; i < nodeDim; i++) {
            I_part.set(stateIdx + i, i, 1);
        }

        // Columns are already in the desired order, so don't mess with those
        I_part.permuteRows(ess.perm);
        Matrix PAinvP = ess.cd.solve(I_part);
        PAinvP.permuteRows(ess.invPerm);

        return PAinvP.copy(stateIdx, stateIdx + nodeDim-1,stateIdx, stateIdx + nodeDim-1);
    }
